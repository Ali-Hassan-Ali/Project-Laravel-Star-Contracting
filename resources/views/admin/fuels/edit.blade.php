                ...
                <form method="post" action="{{ route('admin.fuels.update', $fuel->id) }}">
                    ...
                    @foreach ($numbers as $number)
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="{{ $number }}" class="form-control @error($number) is-invalid @enderror" value="{{ old($number, $fuel[$number]) }}" required autofocus>
                            @error($number)
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    @foreach ($textareas as $textarea)
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control @error($textarea) is-invalid @enderror" name="{{ $textarea }}" rows="6">{{ old($textarea, $fuel[$textarea]) }}</textarea>
                            @error($textarea)
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
