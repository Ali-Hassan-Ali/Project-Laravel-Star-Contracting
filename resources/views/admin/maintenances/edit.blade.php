                ...
                <form method="post" action="{{ route('admin.maintenances.update', $maintenance->id) }}">
                    ...
                    @foreach ($numbers as $number)
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" name="{{ $number }}" class="form-control @error($number) is-invalid @enderror" value="{{ old($number, $maintenance[$number]) }}" required autofocus>
                            @error($number)
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                    ...
                    @foreach ($data_times as $data_time)
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="{{ $data_time }}" class="form-control @error($data_time) is-invalid @enderror" value="{{ old($data_time, date('Y-m-d', strtotime($maintenance[$data_time])) ) }}" required autofocus>
                            @error($data_time)
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                        ...
                        <div class="form-group">
                            ...
                            <select name="{{ $enum }}" class="form-control select2 @error($enum) custom-select @enderror" required>
                                ...
                                @foreach ($status as $statu)
                                    <option value="{{ $statu }}" {{ $statu == old($enum, $maintenance->scheduled) ? 'selected' : '' }}>@lang('site.' . $statu)</option>
                                @endforeach
                            </select>
                            @error($enum)
                                <span class="invalid-feedback" role="alert">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control @error('non_scheduled') is-invalid @enderror" name="non_scheduled" rows="6">{{ old('non_scheduled', $maintenance->non_scheduled) }}</textarea>
                        @error('non_scheduled')
                            <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                            </span>
