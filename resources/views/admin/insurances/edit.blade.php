        ...
        <div class="col-md-12">
            ...
            <div class="tile shadow">
                ...
                <form method="post" action="{{ route('admin.insurances.update', $insurance->id) }}">
                        ...
                        <div class="form-group @error('equipment_id') custom-select @enderror">
                            ...
                            <select name="equipment_id" id="equipment-man" class="form-control select2-tags-false" required>
                                ...
                                @foreach ($equipments as $equipment)
                                    <option value="{{ $equipment->id }}" {{ $equipment->id == old('equipment_id', $insurance->equipment_id) ? 'selected' : '' }}>{{ $equipment->name .' '. $equipment->make .' '. $equipment->plate_no }}</option>
                                @endforeach
                            </select>
                            @error('equipment_id')
                    ...
                    <div class="form-group">
                        <label>
                            @lang('insurances.claim_attachments') 
                            <span class="text-danger">*</span>
                            <small>( @lang('insurances.attachments_mssage')</small>
                            <small style="font-weight: bold;">@lang('insurances.attachments_docum') )</small>
                        </label>
                        <input type="file" name="claim_attachments[]" autofocus multiple class="form-control" accept=".pdf,image/*,.doc,.docx">
                    </div>
