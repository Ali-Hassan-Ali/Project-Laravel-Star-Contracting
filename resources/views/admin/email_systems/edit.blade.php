    ...
    <div class="row">
        ...
        <div class="col-md-12">
            ...
            <div class="tile shadow">
                ...
                <form method="post" action="{{ route('admin.email_systems.update', $EmailSystem->id) }}">
                    @csrf
                    @method('put')
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" name="email" class="form-control @error('email') is-invalid @enderror" value="{{ old('email', $EmailSystem->email) }}" required autofocus>
                        @error('email')
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="country_id" class="form-control col-6 select2" id="equipment-countrey" required>
                                ...
                                @foreach ($countrys as $country)
                                    <option value="{{ $country->id }}" {{ old('country_id', $EmailSystem->country_id) == $country->id ? 'selected' : '' }}
                        ...
                        <div class="form-group col-6">
                            ...
                            <select name="city_id" class="form-control select2" id="equipment-city" required>
                                @foreach ($citys as $city)
                                    <option value="{{ $city->id }}" {{ old('city_id', $EmailSystem->city_id) == $city->id ? 'selected' : '' }}
                                    >{{ $city->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="type" class="form-control select2" required>
                            @foreach($types as $type)
                                <option value="{{ $type }}" {{ old('type', $EmailSystem->type) == $type ? 'selected' : '' }}>@lang('email_systems.'. $type)</option>
                            @endforeach
                        </select>
                    </div>
